...
class LinkCamera extends StatefulWidget {
...
class _LinkCameraState extends State<LinkCamera> {
  API api = API();
  ...
  void initState() {
    super.initState();
    _getcamera(widget.direction.place_name, widget.direction.name);
  }

  void toggleCameraSelection(Camera camera) {
    setState(() {
      if (widget.selectedCameras.any((c) => c.id == camera.id)) {
        widget.selectedCameras.removeWhere((c) => c.id == camera.id);
      } else {
        widget.selectedCameras.add(camera);
      }
    });
  }
  ...
  Future<void> _getcamera(String placename, String directionname) async {
    if (placename.isEmpty || directionname.isEmpty) {
      _showSnackbar('Please enter both place name and direction');
      return;
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _showSnackbar('No Camera found for $placename at point $directionname');
      } else {
        setState(() {
          cameraList = [];
        });
                  ...
                  Align(
                    ...
                    child: Container(
                      ...
                      child: IconButton(
                        ...
                        onPressed: () {
                          Navigator.pop(context, widget.selectedCameras);
                        },
                      ),
                    ),
                  ),
                    ...
                    child: cameraList.isNotEmpty
                        ? ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              Camera camera = cameraList[index];
                              bool isSelected = widget.selectedCameras
                                  .any((c) => c.id == camera.id);
